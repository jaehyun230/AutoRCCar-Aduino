...
class NeuralNetwork():
    def __init__(self):
        self.model = cv2.ml.ANN_MLP_load('mlp_xml/mlp_1504380896.xml')
        # mlp_xml/mlp_1501357468.xml
...
class DistanceToCamera(object):
    ...
    def calculate(self, v, h, x_shift, image):
        ...
        if d > 0:
            cv2.putText(image, "{d:.1f}cm", (image.shape[1] - x_shift, image.shape[0] - 20),
...
class VideoStreamHandler(socketserver.StreamRequestHandler):
    # Cascade Classifiers
    stop_classifier = cv2.CascadeClassifier('D:\\SelfDrivingRCCar-master\\server\\cascade_classifiers\\stop_sign.xml')
    light_classifier = cv2.CascadeClassifier(
        'D:\\SelfDrivingRCCar-master\\server\\cascade_classifiers\\traffic_light.xml')
...
###        global sensor_data
        ...
        try:
            while True:
                ...
                # Object detection
                v_stop = float(self.obj_detection.detect(image, gray, self.stop_classifier))
                v_light = float(self.obj_detection.detect(image, gray, self.light_classifier))

                # Distance measurement
                if (v_stop == None | v_light == None):
                    break

                elif v_stop > 0 or v_light > 0:
...
##                    print("Stopping, obstacle in front!")
                ...
                if 0.0 < self.d_stop < 30.0 and stop_sign_active:
...
class ThreadServer():
 ...
 ##       server.serve_forever()
    ...
    video_thread = threading.Thread(target=server_video_thread, args=('192.168.0.11', 45713))
    video_thread.start()

##   ultrasonic_sensor_thread = threading.Thread(target=ultrasonic_server_thread, args=('192.168.0.11', 45714))
